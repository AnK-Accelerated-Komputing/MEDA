...
from mechdesignagents.custommultimodalagent import CustomMultimodalConversableAgent
...
def create_mechdesign_agents(config):
    ...
    designer_aid = RetrieveUserProxyAgent(
        ...
        retrieve_config={
            ...
            "docs_path": [
                ...
                "data/cadquery-readthedocs-io-en-latest.pdf",
            ],
    ...
    def call_rag(
        question: Annotated[float, "CAD creation methods for which code snippet is to be found"],
    ) -> str:
        """This function calls the RAG agent to find
          the relevant code snippet and imports for using methods
    ...
    cad_image_retriever = AssistantAgent(
        name="CAD_Image_Retriever",
        # is_termination_msg=lambda x: x.get("content", "").rstrip().endswith("TERMINATE"),
        human_input_mode="NEVER",
        code_execution_config=False,
        llm_config=llm_config,
        system_message="Obtain the image path of the CAD model to be reviewed by CAD image reviewer",
        description="The CAD model image reviewer",)
    
    @user.register_for_execution()
    @cad_image_retriever.register_for_llm(
        description="Returns the path of the image of the CAD model to be reviewed by CAD image reviewer")
    def get_image_path() -> str:
        """This function returns the path of the image to be reviewed by the CAD_Image_Reviewer."""
        return get_latest_png()
