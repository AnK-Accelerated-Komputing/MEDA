...
def get_latest_png(directory="/home/niel77/testmeda/MEDA/ptests/results/CAD_Prompts_reviewer_30") -> Optional[str]:
    """
...
def get_image_info(prompt):
    ...
    save_token_usage(prompt, completion_tokens, prompt_tokens, total_tokens)
    
    return response.choices[0].message.content

def save_token_usage(prompt, completion_tokens, prompt_tokens, total_tokens):
    """
    ...
    with open(csv_path, mode='a', newline='') as file:
        ...
        if not file_exists:
            writer.writerow(["Timestamp", "Prompt", "Completion Tokens", "Prompt Tokens", "Total Tokens"])
        
        # Write the data row
        writer.writerow([timestamp, prompt, completion_tokens, prompt_tokens, total_tokens])